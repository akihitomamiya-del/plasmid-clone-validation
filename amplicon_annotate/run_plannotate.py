...
def run_plannotate(fasta, force_linear=False):
    ...
    # When force_linear is set (e.g. amplicons) annotate linearly up front,
    # which skips the doubling and avoids spurious origin-spanning hits.
    if force_linear:
        linear = True
        df = annotate(
        ...
        try:
            linear = False
            df = annotate(
                seq, is_detailed=True, linear=linear,
                yaml_file="plannotate.yaml")
        except IndexError:
            linear = True
            df = annotate(
                seq, is_detailed=True, linear=linear,
...
def create_gbk(sample_file, item, df, is_linear=False):
    """Run plannotate for a sample.

    :param sample_file:
    :param item: the sample name
    :param df: the plannotate df
    :param is_linear: write a linear (not circular) GenBank record
    """
...
def per_assembly(sample_file, item, force_linear=False):
    """Run plannotate for a sample.

    :param sample_file:
    :param item: the sample
    :param force_linear: annotate and write GenBank linearly
    """
    plot, annotations, clean_df = run_plannotate(
        sample_file, force_linear=force_linear)
    create_gbk(sample_file, item, clean_df, is_linear=force_linear)
...
def attempt_annotation(sample_file, name, force_linear=False):
    """Create annotation dictionary for report and EPI2ME."""
    tup_dic, clean_df = per_assembly(
        sample_file, name, force_linear=force_linear)
        ...
        for filename in os.listdir(args.sequences):
            ...
            try:
                tup_dic, report, plannotate_dic = attempt_annotation(
                    file, name, force_linear=args.linear)
...
def argparser():
    ...
    parser.add_argument(
        "--linear",
        action="store_true",
        help="force linear annotation (no circular doubling) and write a "
             "linear GenBank; use for amplicons / linear constructs.")
    return parser
