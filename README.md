...
| `build_arabidopsis_db.sh` + `ARAB_DB` | **Arabidopsis-aware annotation**: build a custom *A. thaliana* protein DB once (outside the firewall; auto-fetches diamond), then amplicon **and** plasmid (`CIRCULAR`) consensuses gain the **AGI locus + gene symbol + function**. Baked into the published image so it's on by default. See [`docs/arabidopsis_annotation_plan.md`](docs/arabidopsis_annotation_plan.md). |
| `validate_against_reference.sh` | **Reference check** ("does my clone match the intended construct?"): align a consensus to a user-supplied `.gbk`/`.fasta` and flag substitutions / indels / truncations, noting which annotated feature each lands in → `variants_vs_reference.csv` + summary. See [`docs/reference_validation.md`](docs/reference_validation.md). |
